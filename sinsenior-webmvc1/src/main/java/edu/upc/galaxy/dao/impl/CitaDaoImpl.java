...
public class CitaDaoImpl extends SimpleJdbcDaoSupport implements CitaDao {
    ...
    public void actualizar(Cita cita) {        
         ...
         getJdbcTemplate().update(
        	     ...
        	     "select codigoInmueble, codigoPersona, ?, 4, 1, ?,?,? from detalleSolicitud where codigo=?",
                 cita.getCodigoUsuario(),fecha,cita.getFecha(),cita.getLugar(), cita.getCodigoDetalle());
        ...
        try {
        	System.out.println(id);
                return getSimpleJdbcTemplate().queryForObject(
                                ...
                                "p.tipoPersona, p.numRazSocial, p.docId,p.telefono,p.celular,p.correo, ds.credat , ds.codigo codigoDetalle, ds.codigoEstado "+
                                "from inmuebles i " +
    ...
    public List<Cita> buscarTodosDisponibles() {
        return getSimpleJdbcTemplate().query(
        	       ...
        	       "p.numRazSocial,p.docID, ds.credat " +
                         ...
                         "left join persona p on p.codigo=ds.codigoPersona " +
                         "left join estado e on e.codigo=ds.codigoEstado " +
                         "where ds.activo=1 and codigoEstado in (1,2) " ,
    ...
    public List<Cita> buscarTodosPendientes() {
        return getSimpleJdbcTemplate().query(
        	       ...
        	       "p.numRazSocial,p.docID, ds.credat, ds.fecha, ds.lugar " +
                         ...
                         "left join persona p on p.codigo=ds.codigoPersona " +
                         "left join estado e on e.codigo=ds.codigoEstado " +
                         "where ds.activo=1 and codigoEstado in (3,4) " ,
                         new BeanPropertyRowMapper<Cita>(Cita.class));
    }
